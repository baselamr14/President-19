...
class AssetAdjustmentWizard(models.TransientModel):
    ...
    _description = 'Asset Adjustment Wizard'

    def _get_default_message(self):
        if self.env.user.lang == 'ar_001':
            return "اذا كنت تستخدم جهاز الليزر لجرد الأصل، يمكنك اختيار 'يدوي' والمسح باستخدام الليزر"
        return "If you are using a Handheld device, you may select the 'Manual' option, and scan using the laser beam."

    adjustment_id = fields.Many2one(comodel_name="asset.adjustment")
    adjustment_method_web = fields.Selection(
        string="Method",
        selection=[('manual', 'Manually'), ('import', 'Excel Import')],
        required=True,
        default='manual'
    )
    adjustment_method = fields.Selection(
        string="Method",
        selection=[('manual', 'Manually'), ('import', 'Excel Import'), ('qr_scan', 'QR Scanning')],
        required=True,
        default='manual'
    )
    ...
    def _set_adjustment_method(self):
        ...
        if device_type not in ['ipad', 'iphone', 'android']:
    ...
    def _check_asset_code(self, code):
        ...
        if asset_line and asset_line.asset_location_id:
            asset_line.write({'asset_code': code, 'is_exist': True})

        elif asset_line and (
            asset_line.asset_id.location_id.id == self.adjustment_id.location_id.id
            or asset_line.asset_id.location_id.id in self.adjustment_id.location_id.child_ids.ids
        ):
            asset_line.write({
                'asset_code': code,
            ...
            if asset:
                self.env['asset.adjustment.line'].create({
                    'asset_id': asset.id,
                    'asset_code': code,
    ...
    def generate_sample_excel_file(self):
        ...
        sheet.write(1, 0, 'TV/0001')
        sheet.write(2, 0, 'TV/0002')
        sheet.write(3, 0, 'TV/0003')
        ...
        return {
            ...
            'url': f'/web/content/asset.adjustment.wizard/{self.id}/sample_excel_file/SampleSheet.xlsx?download=true',
            'target': 'self'
        }
    ...
    def check_import_excel_file(self):
        for rec in self:
            if rec.file_name:
                ...
                if ext != "xlsx":
                    raise ValidationError(_("Sorry File extension should be .xlsx !"))

    def _excel_cell_to_code(self, value):
        """Normalize Excel cell to a usable asset code string."""
        if value is None:
            return ''
    ...
    def import_adjustment_sheet(self):
        ...
        if not openpyxl:
            raise ValidationError(_("openpyxl is not installed on the server. Add it to requirements.txt."))

        data = base64.b64decode(self.excel_file)
        wb = openpyxl.load_workbook(filename=BytesIO(data), data_only=True)
        ws = wb.active

        # Read first column, starting from row 2 (skip header)
        for row in ws.iter_rows(min_row=2, min_col=1, max_col=1):
            code = self._excel_cell_to_code(row[0].value)
            if code:
                self._check_asset_code(code)

    def action_done(self):
        if self.adjustment_method == 'manual':
            self._check_asset_code(self.code)
        elif self.adjustment_method == 'qr_scan' and self.asset_id:
...
#     @api.constrains('adjustment_method_web')
#     def _set_adjustment_method(self):
#         # Odoo 19 compatible: request may be unavailable outside HTTP context
#         device_type = False
#         if request and getattr(request, "httprequest", None):
#             device_type = request.httprequest.user_agent.platform
#
...
#             'type': 'ir.actions.act_url',
#             'name': 'Sample Adjustment Sheet',
#             'url': '/web/content/asset.adjustment.wizard/%s/sample_excel_file/SampleSheet.xlsx?download=true' % (self.id),
#             'target': 'self'
#         }
#
#     @api.constrains('file_name')
